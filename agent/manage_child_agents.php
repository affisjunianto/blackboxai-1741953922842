...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        redirect('/agent/manage_child_agents.php');
    }
    ...
    try {
        switch ($action) {
            case 'create':
                ...
                // Create child agent with current agent as parent
                $stmt = $pdo->prepare("
            ...
            case 'update':
                $childAgentId = (int)$_POST['child_agent_id'];
                $email = sanitizeInput($_POST['email']);
                ...
                $stmt = $pdo->prepare("
                    ...
                    WHERE id = ? AND parent_id = ? AND role = 'agent'
                ");
                $stmt->execute([$childAgentId, $_SESSION['user_id']]);
                if (!$stmt->fetch()) {
                ...
                $stmt->execute([$email, $childAgentId]);
                ...
                if (!empty($newPassword)) {
                    ...
                    $stmt->execute([$hashedPassword, $childAgentId]);
                }
...
// Fetch agent's child agents
try {
    $stmt = $pdo->prepare("
        SELECT * FROM users 
        WHERE parent_id = ? AND role = 'agent' 
        ORDER BY username
    ");
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editAgentForm" method="POST" class="needs-validation" novalidate>
                    ...
                    <input type="hidden" name="child_agent_id" id="editAgentId">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST">
                    ...
                    <input type="hidden" name="child_agent_id" id="deleteAgentId">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
